...
/** "Continue with Google" — paper pill with the Google mark; navigates away on tap. */
export function GoogleButton({ onError }: { onError?: (msg: string) => void }) {
  const [busy, setBusy] = useState(false)
  async function go() {
    setBusy(true)
    const { error } = await signInWithGoogle()
    if (error) { setBusy(false); onError?.(error) }
    // On success the browser is redirected; nothing more to do here.
  }
  return (
    <button type="button" onClick={go} disabled={busy} className="btn light flex items-center justify-center gap-2.5">
